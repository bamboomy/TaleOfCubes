...
import static org.bamboomy.taleofcubes.game.BeastRenderer.MATRIX_SIZE;
import static org.bamboomy.taleofcubes.game.Cube.NUMBER_OF_SIDES;
...
public class Beast {
    ...
    Beast(MotionListener motionListener, BeastActivity beastActivity) {

        for (int i = 0; i < cubes.length / 2; i++) {
            cubes[i] = new Cube(motionListener, beastActivity, true);
        }

        for (int i = cubes.length / 2; i < cubes.length; i++) {
            cubes[i] = new Cube(motionListener, beastActivity, false);
        }
    ...
    private void initCubes() {
        ...
        float[] translateMatrix = new float[MATRIX_SIZE];
        ...
        translateMatrix = new float[MATRIX_SIZE];
        Matrix.setIdentityM(translateMatrix, 0);
