...
import static org.bamboomy.taleofcubes.game.GameMaster.TAG;
...
class Side {
    ...
    Side(BeastActivity beastActivity, boolean chooseUnchosen) {

        textures = new int[1];

        if (COVER == null) {
            COVER = Pictures.decodeSampledBitmapFromResource(
                    R.drawable.questionmark, 200, 200, beastActivity);
        }

        if (GOOD == null) {
            GOOD = Pictures.decodeSampledBitmapFromResource(
                    R.drawable.good, 200, 200, beastActivity);
        }
        ...
        if (tupple.isNew()) {
            ...
            tupple.setPicture(Pictures.getUnchoosenPicture(beastActivity));
        }
